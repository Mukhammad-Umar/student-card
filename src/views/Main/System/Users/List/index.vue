...
const fields = computed(() => [
  ...
  { key: 'roleName', label: t('role'), class: 'text-center' },
...
async function getList() {
  try {
    loading.value = true
    const data = await orgUserStore.getList({
    ...
    rows.value = data
    filterData.pagination.rowsNumber = orgUserStore.totalPages
    filterData.pagination.totalCount = orgUserStore.totalCount
    ...
    <b-col>
      ...
      <b-card no-body class="card-box-shadow">
        <b-card-header class="align-items-center d-flex justify-content-between">
          ...
          <b-button variant="info" :to="{ name: 'MainOrgsUsersAdd' }">
        ...
        <div class="table-responsive mb-0">
          <b-table :items="rows" :fields="fields" :busy="loading" :show-empty="!loading" hover>
            ...
            <template #cell(firstName)="{ item }">{{ item.lastName + ' ' + item.firstName }}</template>

            <template #cell(phone)="{ item }">{{ filters.filterFullPhoneNumber(item.phone) }}</template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="d-flex justify-content-center">
                <ResetPassword :userId="item.userId"/>

                <i
                  class="mdi mdi-pencil-outline text-info cup h3 ml-1"
                  @click="$router.push({ name: 'MainOrgsUsersEdit', params: { id: item.userId } })"
                ></i>
              </div>
            </template>
          </b-table>
