...
const fields = computed(() => [
  ...
  { key: 'roleName', label: t('sysUsers.createRoleName'), class: 'text-center' },
  { key: 'organizationBranchName', label: t('filters.branch'), class: 'text-center' },
  { key: 'createDate', label: t('filters.date'), class: 'text-center' },
...
async function getList() {
  try {
    loading.value = true
    const data = await roleStore.getList({
      SearchText: filterData.search || null,
    ...
    rows.value = data
    filterData.pagination.rowsNumber = roleStore.totalPages
    filterData.pagination.totalCount = roleStore.totalCount
    ...
    <b-col>
      ...
      <b-card no-body class="card-box-shadow">
        <b-card-header class="align-items-center d-flex justify-content-between">
          ...
          <b-button variant="info" :to="{ name: 'MainOrgsRolesAdd' }">
          ...
          <b-table :items="rows" :fields="fields" :busy="loading" :show-empty="!loading" hover>
            ...
            <template v-slot:cell(actions)="{ item }">
              <i
                ...
                @click="$router.push({ name: 'MainOrgsRolesEdit', params: { id: item.roleId } })"
              ></i>
            </template>
          </b-table>
        </div>
